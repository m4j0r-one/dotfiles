#!/usr/bin/env bash
# Komplettbackup fuer /home/m4j0r/scripts
# Installationspfad: /home/m4j0r/scripts/scripts-backup.sh
# Backup-Ziel:       /home/m4j0r/Backup/Scripts
...
readonly SOURCE_DIR="${USER_HOME}/scripts"
readonly BACKUP_ROOT="${USER_HOME}/Backup/Scripts"
readonly INSTALLED_SCRIPT="${SOURCE_DIR}/scripts-backup.sh"
...
create_restore_readme() {
    local target="$1"
    cat > "$target" <<'README'
SCRIPTS-BACKUP – WIEDERHERSTELLUNG
=================================

Empfohlen:
1. ~/scripts/scripts-backup.sh starten.
...
Script-Ordner installiert sind.

Vor jeder Wiederherstellung wird automatisch ein Sicherheitsbackup des
aktuellen Script-Ordners erstellt. Vorhandene Dateien werden ersetzt,
...
get_backups() {
    find "$BACKUP_ROOT" -maxdepth 1 -type f \
        \( -name 'scripts-backup-*.tar.zst' -o -name 'scripts-backup-*.tar.gz' \) \
        -printf '%T@ %p\n' 2>/dev/null \
        | sort -nr \
        | cut -d' ' -f2-
}
...
prune_old_backups() {
    ...
    shopt -s nullglob
    for checksum in "$BACKUP_ROOT"/scripts-backup-*.tar.zst.sha256 \
                    "$BACKUP_ROOT"/scripts-backup-*.tar.gz.sha256; do
...
create_backup() {
    ...
    cleanup_dir="$(mktemp -d "${BACKUP_ROOT}/.scripts-backup-tmp.XXXXXX")"
    ...
    if command -v zstd >/dev/null 2>&1; then
        extension="tar.zst"
        archive="${BACKUP_ROOT}/scripts-backup-${timestamp}-${hostname_safe}.${extension}"
...
list_backups() {
    ...
    if (( ${#backups[@]} == 0 )); then
        printf 'Noch keine Scripts-Backups vorhanden.\n'
        return 1
    fi
...
restore_backup() {
    ...
    printf 'Zusaetzliche Dateien im Script-Ordner werden nicht geloescht.\n\n'
    ...
    cleanup_dir="$(mktemp -d "${BACKUP_ROOT}/.scripts-restore-tmp.XXXXXX")"
    tar -xpf "$archive" -C "$cleanup_dir"

    if [[ ! -d "${cleanup_dir}/payload/home/m4j0r/scripts" ]]; then
        printf '\nUngueltiges Backup: payload/home/m4j0r/scripts fehlt.\n' >&2
        return 1
    fi
    ...
    printf '\nStelle Script-Ordner wieder her ...\n'
    mkdir -p "$SOURCE_DIR"

    if command -v rsync >/dev/null 2>&1; then
        rsync -a --human-readable --info=NAME \
            "${cleanup_dir}/payload/home/m4j0r/scripts/" "${SOURCE_DIR}/"
    else
        cp -a -- "${cleanup_dir}/payload/home/m4j0r/scripts/." "$SOURCE_DIR/"
    fi

    find "$SOURCE_DIR" -maxdepth 1 -type f -name '*-backup.sh' -exec chmod 755 {} + 2>/dev/null || true
